...
public class ReservaService {
    ...
    public double calcularFianca(Client client, Vehicle vehicle) {
        ...
        if (client.getReputacio() == Reputacio.PREMIUM) {
            return fiancaStandard * 0.75; // 25% descuento para clientes PREMIUM
        } else {
            return fiancaStandard; // Clientes NORMAL pagan la fianza estándar
        }
    }*/
    ...
    public double calcularCostReserva(LocalDateTime dataInici, LocalDateTime dataFin, double preuPerHoraLloguer) {
        long hores = ChronoUnit.HOURS.between(dataInici, dataFin);
        if (hores <= 0) {
            throw new IllegalArgumentException("La data de finalització ha de ser posterior a la data d'inici.");
        }
